...
// Create a copy of the kernel page table and return it.
pagetable_t kpagetable()
{
  ...
  // CLINT
  kvmmap(kpt, CLINT, CLINT, 0x10000, PTE_R | PTE_W);
...
void freekpagetable(int level, pagetable_t pagetable)
{
  if (level <= 2)
  {
    for (int i = 0; i < 512; i++)
    {
      ...
      if (pte & PTE_V)
      {
        freekpagetable(level + 1, (pagetable_t)PTE2PA(pte));
      }
    }
  }
...
freeproc(struct proc *p)
{
  ...
  // free the physical page of kernel stack
  void *stackaddr = (void *)kvmpa(p->kernelpagetable, p->kstack);
  kfree(stackaddr);
  p->kstack = 0;

  // free process kernel page table
  if (p->kernelpagetable)
    freekpagetable(1, p->kernelpagetable);
...
void userinit(void)
{
  ...
  copymapping(p->pagetable, p->kernelpagetable, 0, p->sz);
...
int growproc(int n)
{
  ...
  if (n > 0)
  {
    ...
    copymapping(p->pagetable, p->kernelpagetable, p->sz, sz);
  }
  else if (n < 0)
  {
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  copymapping(np->pagetable, np->kernelpagetable, 0, np->sz);
